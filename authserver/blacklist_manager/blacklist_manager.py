...
class JWTBlacklistManager(object):
    ...
    def _redis_value_exists(self, key=""):
        try:
            ...
            current_app.logger.error("Can't connect to the Redis shared cache.")
            raise RedisConnectionError("Can't connect to the Redis shared cache.")
        except redis_exceptions.RedisError:
            current_app.logger.error("Can't execute the EXISTS method at the Redis shared cache.")
    ...
    def _get_redis_value(self, key=""):
        try:
            ...
            current_app.logger.error("Can't execute the GET method at the Redis shared cache.")
    ...
    def _set_redis_value(self, key, value, expiration_time):
        try:
            ...
            current_app.logger.error("Can't execute the SET method at the Redis shared cache.")
    ...
    def _delete_redis_value(self, key):
        try:
            ...
            current_app.logger.error("Can't execute the DELETE method at the Redis shared cache.")
    ...
    def _get_token_data_dict(data_str: str):
        if data_str:
            try:
                ...
                current_app.logger.error("Error decoding active token information. Details: {}".format(str(e)))
                return None
        else:
            return None
    ...
    def _check_token_type(token, expected_type):
        # Return true if the token type meets the expected one
    ...
    def _add_access_token(self, access_token):
        ...
        if self._get_redis_value(access_jti) is not None:
            current_app.logger.warning("Token already registered in the active tokens list.")
            return

        # Save the token to the Redis shared cache
        self._set_redis_value(access_jti, access_data, self.access_expiration_time * 1.2)
        current_app.logger.info("Access token (jti: '{}') saved to the active token list.".format(access_jti))

        return access_jti

    def _add_refresh_token(self, refresh_token, access_jti):
        # Get the token jti and the token data
        ...
        current_app.logger.info("Refresh token (jti: '{}') saved to the active token list.".format(refresh_jti))
    ...
    def add_token_set(self, access_token, refresh_token, access_encoded=True, refresh_encoded=True):
        ...
        access_jti = self._add_access_token(access_token)
        self._add_refresh_token(refresh_token, access_jti)
    ...
    def revoke_access_token(self, token, encoded=True):
        ...
        if deleted_count == 0:
            current_app.logger.warning("Access token (jti: '{}') to revoke not found in the Redis cache."
                                       "Considering it already revoked.".format(token_jti))
        else:
            current_app.logger.info("Access token (jti: '{}') successfully revoked from the active list.".
                                    format(token_jti))
    ...
    def revoke_refresh_token(self, token, encoded=True, revoke_associated_access_token=True):
        ...
        if token_data:
            ...
            current_app.logger.info("Refresh token (jti: '{}') successfully revoked from the active list.".
                                    format(token_jti))
        else:
            # Print a warning if the token was already revoked (not found in the Redis cache)
            current_app.logger.warning("Refresh token (jti: '{}') to revoke not found in the Redis cache."
                                       "Considering it already revoked.".format(token_jti))
            return

        if revoke_associated_access_token:
            self.revoke_access_token({"jti": token_data.get("access_jti")}, encoded=False)
    ...
    def update_refresh_associated_access_token(self, refresh_token, new_access_token, access_encoded=True,
        ...
        if new_expiration_time.total_seconds() == 0:
            current_app.logger.info("The remaining refresh token (jti: '{}') expiration time is 0."
        ...
        new_expiration_time += self.refresh_expiration_time * 1.2

        # Revoke the old access token before setting up the new one
        self.revoke_access_token({"jti": refresh_token_data.get("access_jti")}, encoded=False)

        # Add the new access token to the token active list
        new_access_token_jti = self._add_access_token(new_access_token)
        ...
        current_app.logger.info("Refresh token (jti: '{}') associated access token successfully updated.".
